...
void MessageSequence::pushNotification(JottingNotification newNotification)
{
    ...
    for(auto& recipient:newNotification.recipients()){
        std::string com_relation="insert into JotNotificationRelation (JN_id,Recipient_id) values("+newNotification.id()+","+recipient.first+")";
...
void MessageSequence::removeNotification(std::string id)
{
    ...
    std::string com_relation="delete from JotNotificationRelation where JN_id="+id;
    RelationalBroker::drop(com_relation);

    std::string com="delete from JotNotification where JN_id="+id;
...
void MessageSequence::updateMessageQueue()
{
    for(auto& message:m_messageQueue){
        ...
        while (res->next()) {
            std::string id=std::to_string(res->getInt(1));
            if(message.second.recipients().count(id)==0){
                std::string com_relation="delete from JotNotificationRelation where JN_id="+message.first+"and Recipient_id="+id;
                RelationalBroker::drop(com_relation);
            }
        }
    }
...
MessageSequence::MessageSequence()
{
    ...
    for(auto& message:m_messageQueue){
        std::string com_relation="select Recipient_id from JotNotificationRelation where JN_id="+message.first;
        std::string recipientId;
        res=RelationalBroker::query(com_relation);
        while (res->next()) {
            recipientId=std::to_string(res->getUInt(2));
